...
import { normalizeNativeAssetAmountUnit } from 'utility/buildInOutDisplay'
...
class NormalTxForm extends React.Component {
  ...
  estimateNormalTransactionGas() {
    ...
    const gasAction = {
      accountAlias,
      accountId,
      assetAlias: NativeAssetAmountUnit[0],
    ...
    this.connection.request('/build-transaction', body).then(resp => {
      ...
      return this.connection.request('/estimate-transaction-gas', {
        ...
        this.setState({estimateGas: Math.ceil(resp.data.totalUny/100000)*100000})
      })
    })
  }
  ...
  render() {
    ...
    const showNativeAssetAmountUnit = (assetAlias.value === NativeAssetAmountUnit[0] || assetId.value === NativeAssetID)
                    ...
                    <td>
                      {
                        ...
                        && this.state.estimateGas && ((lang === 'zh' ? '估算' : 'estimated') + '   ' + normalizeNativeAssetAmountUnit(NativeAssetID,
                          option.ratio * this.state.estimateGas,
                          this.props.nativeAssetAmountUnit))
                      }
                      {
